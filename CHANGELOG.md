...
- ロール定義ファイル `ROLES.md` を追加
  - `Tester（テスト実行者）`
  - `TestDesigner（テスト設計者）`
  - `TestAutomationEngineer（テスト自動化設計者）`
  - `Developer（開発者）`
- ロール命名を英語識別子 + 日本語補足で統一
- `※JSTQB内での定義準拠（本プロジェクト向けに用語を整理）` を明記

### 🔄 テスト運用整理

- `test/` 配下を目的別に再編
  - `test/DevTest/`: 教材・フロント回帰（headed中心）
  - `test/Other/`: API確認の補助スクリプト
